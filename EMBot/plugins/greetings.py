...
@evil.on(events.CallbackQuery(pattern=r"check-bot-(\d+)"))
async def check(event):
    ...
    if event.sender_id == user_id:
            await evil.edit_permissions(chat_id, event.sender_id, send_messages=True)
...
@evil.on(events.ChatAction)
async def join(event):

    if event.user_joined:
        await evil.edit_permissions(event.chat_id, event.user_id, send_messages=False)
